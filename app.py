...
if not st.session_state.autenticado:
    st.markdown("---")
    st.markdown("### 🔐 Acesso ao Relatório")
    ...
    with st.expander("🟢 CULTO DAS 09:00h", expanded=False):
        ...
        st.markdown('<div class="pib-faixa">🎒 MI / 🎮 MPA</div>', unsafe_allow_html=True)
    ...
    with st.expander("🟢 CULTO DAS 11:00h", expanded=False):
        ...
        st.markdown('<label>Data (Sincronizada)</label>', unsafe_allow_html=True)
        ...
        mic11, mis11, mip11 = mi4.number_input("Crianças ", 0, key="mic11"), mi5.number_input("Servos ", 0, key="mis11"), mi6.number_input("Pais ", 0, key="mip11")
    ...
    rel_final = f"""Relatório de Contagem - PIB Floripa
Data: {d_rel}

CULTOS DAS 09:00H
Responsável: {r9}
Templo/Visit/6º: {t9}/{v9}/{s9} (Total: {t_c9})
Servindo: {t_s9} | MI: {t_mi9} | MPA: {t_mpa9} | Ebed: {eb9}
TOTAL 09:00H: {total_9h}

CULTOS DAS 11:00H
Responsável: {r11}
Templo/Visit/6º: {t11}/{v11}/{s11} (Total: {t_c11})
Servindo: {t_s11} | MI: {t_mi11} | Batismo: {t_bat}
TOTAL 11:00H: {total_11h}

TOTAL GERAL DOMINGO: {total_9h + total_11h}"""

    st.markdown("---")
    cv1, cv2 = st.columns(2)
    if cv1.button("📄 Ver Resumo"): st.session_state.mostrar_relatorio = not st.session_state.mostrar_relatorio
    if cv2.button("📲 WhatsApp"): st.session_state.wa_confirmar = not st.session_state.wa_confirmar
